/**
 * gen-sitemap-full.js — Full filesystem sitemap for fixlifyservices.com
  ...
  for (const item of items) {
    if (item.isDirectory()) {
      ...
      if (!SKIP_FILES.has(item.name) && !item.name.startsWith('landing-')) {
        const fullFilePath = path.join(dir, item.name);
        if (hasNoindex(fullFilePath)) continue;
...
console.log(`Sitemap: ${files.length} URLs → fixlifyservices.com/sitemap.xml`);
