 ...
 * Cross-platform, uses __dirname for path resolution
 */
const fs = require('fs');
...
function getLastmod(filename) {
  ...
  if (filename.match(/^(about|contact|book|pricing|services|locations|brands|emergency)\.html$/)) return '2025-11-25';
  return '2026-01-15';
}
...
function getPriority(filename) {
  ...
  if (filename.match(/^(about|book|contact)\.html$/)) return '0.6';
  return '0.6';
}
...
console.log(`Sitemap generated: ${urls.length} URLs → fixlifyservices.com/sitemap.xml`);
