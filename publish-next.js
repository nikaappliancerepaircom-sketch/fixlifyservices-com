...
async function main() {
  ...
  if (!fs.existsSync(QUEUE_DIR)) { console.log('No _queue directory. Nothing to publish.'); return; }
  const files = fs.readdirSync(QUEUE_DIR).filter(f => f.endsWith('.html')).sort();
  if (files.length === 0) { console.log('Queue is empty. Nothing to publish today.'); return; }
  const file = files[0];
  ...
  console.log('\nPublished: ' + file + '\nRemaining in queue: ' + (files.length - 1));
}
