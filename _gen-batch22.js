...
function getProblemCards(svcType, brands) {
  ...
  return [
    ...
    ['Unusual noises', 'Evaporator fan, condenser fan, or compressor vibration. Noise diagnosis included in $89 diagnostic.']
  ];
}
...
function buildPage(n, svc) {
  ...
  const faqSchema = JSON.stringify({
    ...
    "mainEntity": [
      ...
      {"@type":"Question","name": "How much does " + svcLabel.toLowerCase() + " repair cost in " + name + "?","acceptedAnswer":{"@type":"Answer","text": "Typical " + svcLabel.toLowerCase() + " repair in " + name + " runs " + cost + " CAD. Flat $89 diagnostic waived with repair. 90-day warranty."}},
  ...
  const bizSchema = JSON.stringify({
    ...
    "description": svcLabel + " Repair in " + name + ", Edmonton. Same-day service, $89 diagnostic, 90-day warranty.",
...
<meta name="description" content="${svcLabel} Repair in ${name}, Edmonton — same-day service, flat $89 diagnostic, 90-day warranty. All brands serviced. Book online or email edmonton@fixlifyservices.com.">
...
<meta property="og:description" content="Same-day ${svcLabel.toLowerCase()} repair in ${name}, Edmonton. From $89 diagnostic, 90-day warranty.">
...
<label class="lz-quick-field"><span>Neighbourhood</span><input type="text" name="city" maxlength="80" value="${name}, Edmonton" autocomplete="address-level2"></label>
        <label class="lz-quick-field"><span>What needs fixing?</span><textarea name="issue" maxlength="1900" rows="3" placeholder="e.g. ${n.brand_chips[0]} ${svcLabel.toLowerCase()} not working"></textarea></label>
...
<main class="page-main container" id="main-content">
  <div class="fade-in" style="max-width:760px;font-size:1.0625rem;color:#374151;line-height:1.75;margin-bottom:56px">
    ...
    <p>Who fixes ${svcLabel.toLowerCase()}s in ${name}? <strong>Fixlify Appliance Services Edmonton</strong> &mdash; same-day ${svcLabel.toLowerCase()} repair throughout ${name} and surrounding Edmonton communities. Book online or email <a href="mailto:edmonton@fixlifyservices.com">edmonton@fixlifyservices.com</a>. Flat <strong>$89 diagnostic</strong>, Mon&ndash;Sat 8AM&ndash;8PM, Sun 10AM&ndash;6PM Mountain Time. Typical cost: <strong>${cost} CAD</strong>. <strong>90-day warranty</strong> on every repair.</p>
    <p>${mainPara}</p>
  ...
  <section style="margin-top:56px">
    ...
    <p style="max-width:760px;color:#374151;line-height:1.7;margin-bottom:20px">All ${name} repairs start with a flat <strong>$89 diagnostic</strong>, waived when you proceed with the repair. Written quote before any work begins.</p>
